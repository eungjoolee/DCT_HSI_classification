...
# DCT
group_dct = parser.add_argument_group("DCT")
group_dct.add_argument(
    "--band_group",
    type=int,
    default=0,
    help="Divide band into several groups for DCT (default = 0 -> Don't use DCT)",
)
group_dct.add_argument(
    "--use_freq",
    type=int,
    default=1,
    help="How many frequencies to use (default = 1 -> DC values)",
)
group_dct.add_argument(
    "--use_kernel",
    type=int,
...
for run in range(N_RUNS):
    ...
    if MODEL == "SVM_grid":
        ...
        if VAL_GT is not None:
            ...
            val_gt = open_file(VAL_GT)['val_gt']
        else:
            train_gt, val_gt = sample_gt(train_gt, 0.9, mode="random")
            # save current split
            scipy.io.savemat("train.mat", {'train_gt': train_gt})
            scipy.io.savemat("val.mat", {'val_gt': val_gt})
